...
2. **Read `specs/REVIEW.md`** — if it exists, the reviewer found issues. Fix blockers BEFORE doing new work.
3. **Read `specs/PLAN.md`** — find the current milestone's first unchecked task
4. **Read `specs/ARCHITECTURE.md`** — only if you need to check a constraint or schema
5. **Do ONE task.** Write code, run it, verify it works.
6. **Run the milestone's validation gate** if you just finished the last task in a milestone
7. **Update `specs/STATUS.md`** — record what you did, any decisions, any issues found
8. **Mark completed tasks** with `[x]` in `specs/PLAN.md`
9. **Exit.** The loop restarts you. Don't try to do the next task.

## Rules

- **ONE task per iteration.** You have ~30 tool calls. Use them on one thing done well.
...
- **Keep `specs/STATUS.md` under 50 lines.** Replace the "Current Task" section each time — don't append iteration history. The progress table and decisions section are the only things that grow.
...
| `specs/REVIEW.md` | Reviewer findings — blockers, warnings, nits | EVERY iteration (right after STATUS.md). Fix blockers before new work. |
...
- **Screenshots are not enough.** When a validation gate says "Playwright screenshot", also use Playwright to check that key elements are visible and have non-zero rendered size. A screenshot of a crushed or clipped element still looks like a "pass" — DOM assertions don't lie.
