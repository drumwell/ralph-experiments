...
# Playwright DOM checks: summary cards visible (≥4), table rows visible (≥5 with non-zero height), charts rendered
pkill -f "node server.js"
```
...
# Playwright DOM checks: table rows still visible (≥5), sidebar present, summary cards have content (not "0" or empty)
pkill -f "node server.js"
```
...
# Playwright DOM checks at each breakpoint: table rows visible, no content clipped by overflow, cards stack on mobile
pkill -f "node server.js"
```
...
**Validation gate:**
Run every check from specs/SPEC.md "Done When". Take screenshots at 1280px. Use Playwright to verify: table shows ≥5 rows with non-zero height, all summary cards have numeric content, charts render, modal opens on row click. If ALL pass, output `<promise>COMPLETE</promise>`.
