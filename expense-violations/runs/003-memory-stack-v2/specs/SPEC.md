...
9. Drill-down modal: clicking a violation row opens a detail panel showing all transaction fields and which rules fired
10. Interactive filters: filter violations table by rule type, severity, date range, merchant
11. Search: real-time search across merchant name, cardholder, card name, amount
12. Export: "Download CSV" button exports current filtered view as CSV
13. Data refresh: "Refresh Data" button re-fetches from Extend API without restarting server
14. Violation trends: line chart showing violation count per day over time
15. Top offenders: card showing top 5 cardholders by violation count
16. Pagination: violations table paginates at 25 rows per page with prev/next controls
17. No dead UI: every element that looks clickable must actually do something (see specs/ARCHITECTURE.md "Cosmetic vs Functional UI Elements")
