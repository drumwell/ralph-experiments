...
- [ ] `/api/violations` must return results sorted by date descending by default
...
- [ ] Add clickable column headers that toggle sort direction (with visual sort indicator)
...
- [ ] Implement top header: logo area, "Card Transactions" title, decorative elements (see specs/ARCHITECTURE.md "Cosmetic vs Functional")
- [ ] Implement search/filter bar: search input, filter/download icons
...
- [ ] Ensure all interactive elements have hover/active states (but NOT decorative elements)
- [ ] Verify refresh does NOT create duplicate records (re-run fetch-data.py and check)
...
- [ ] Take final verification screenshots at all 3 widths (save to screenshots/)
- [ ] Verify no duplicate transaction IDs in data/transactions.json
- [ ] Verify no dead buttons (every element with cursor:pointer has a handler)
