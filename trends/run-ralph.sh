...
LOG_DIR="logs"
# Auto-increment run name: find highest NNN in runs/NNN-*, bump by 1
...
# ── The Loop ─────────────────────────────────────────────────────────────────
# Disable errexit — the loop handles all errors explicitly (rollback, etc.)
set +e
while true; do
    # Increment iteration
    ...
    if [[ ! -f specs/fix_plan.md ]]; then
        ...
        if grep -q "<promise>COMPLETE</promise>" "$BUILD_LOG" 2>/dev/null; then
            ...
            cp -f data/transactions.json data/triage.json "runs/${RUN_NAME}/output/data/" 2>/dev/null || true
